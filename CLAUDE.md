...
> Destructive-action gates and Firebase/Firestore hard stop → see **INVARIANTS.md**.
...
```
Examples: `` `grep -c "export default" frontend/src/pages/index.tsx` → 1 `` | `` `npm run build` → exit 0 ``
...
- T3: `grep -n "Symbol" backend/src/path/to/file.ts` or `frontend/src/path/to/file.ts`
...
3. Confirm → เสนอตัวร่างรายละเอียด (Symptom, Root Cause, และ Correct Resolution) ให้ผู้ใช้ช่วยตรวจสอบความถูกต้องและขอข้อมูลเพิ่มเติม จากนั้นเมื่อได้รับการยืนยันและเห็นชอบจากผู้ใช้แล้ว จึงค่อยเขียนบันทึกเข้าระบบลงไฟล์ `CODING_FAILURE_PATTERNS.md` พร้อมกำหนดรหัส CFP ถัดไป → ย้ายไฟล์ร่างไปที่ `knowledge/cfp-proposals/applied/`
...
- `TESTING.md` — per-action verification matrix (Next.js + Express + Firebase). Load when: task involves API route, Firestore operation, auth, CSV, or new component.

**Safety:**
- ห้ามแก้ไขหรือเขียนเพิ่มใน `CODING_FAILURE_PATTERNS.md` โดยตรงเด็ดขาด — ต้องผ่านกระบวนการเสนอตัวร่าง สอบถามความถูกต้อง และรอการยินยอม (User Confirmation) จากผู้ใช้ผ่านแชตก่อนเสมอ (ป้องกันเอเจนต์คาดเดาความถูกต้องของ Resolution เอง)
...
Add-Content -Path .sessions/cfp_flags.md -Value "$(Get-Date -Format 'yyyy-MM-dd') | skill: <current_skill> | issue: <one-line description of what was wrong>"
```
...
- **Firebase/Firestore Rules:** Do NOT modify firestore.rules or firestore.indexes.json unless specifically instructed. Any database schema updates or queries on the frontend must align with backend API services.
- **Express Backend Type-Safety:** Keep backend strictly type-safe. Run `npm run type-check` to verify compiling.
- **Styling constraints:** Styling must use Vanilla CSS. Avoid TailwindCSS unless explicitly requested by the user.
