...
REOPENED (2026-07-13 · T-054): 4th recurrence in a PLUGIN-ONLY project proves the T-263 gate is NOT effective when local scripts/ are detached to the engine and/or review_intent.py's trigger lexicon misses Thai 'ตรวจสอบ' (verify/check). Fix-escalated. Needed: (a) confirm skill_gate.py + review_intent.py are actually registered as hooks in the plugin engine's settings for plugin-only projects; (b) extend review_intent.py trigger lexicon to include Thai verify verbs 'ตรวจสอบ/เช็ค/ตรวจ' + EN 'verify/check', not only 'review/audit'; (c) as an agent-side backstop until (a)/(b) land: on ANY user verify/check request targeting finished work, load scrutinize (finished artifact) or skeptical_reviewer (plan) BEFORE running bash/grep verification.
status: fix-escalated (REOPENED 2026-07-13 · T-054 · count=4 · shipped T-263 fix ineffective in plugin-only deployment)
