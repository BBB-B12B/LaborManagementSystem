...
- Is there a simpler path that achieves the same outcome? → deep simpler-way pass is owned by the `scrutinize` skill (single source of truth); apply only the one-line check here.
- **Already-rejected check (T-224):** grep `knowledge/out_of_scope.md` for the feature's keywords (`grep -i "<keyword>" knowledge/out_of_scope.md`). HIT → this design was already settled as out-of-scope → emit `[already-rejected] <slug>` + cite the stored `rationale:` → verdict `reject` unless the user explicitly overrides (then update/remove the entry). On-demand grep ONLY — never always-load the file.

Flag: `[unnecessary] Section N — reason` · `[already-rejected] <slug> — reason`
...
**Append-on-reject (T-224):** when a `reject` verdict settles that a feature should NOT be built (a necessity failure — not a fixable gap), append a `### <slug>` entry to `knowledge/out_of_scope.md` (problem / rationale / refs / keywords / date) so the decision is not reopened later. Skip the append if the reject was a fixable flaw — that is a `revise`, not a permanent out-of-scope ruling.
