...
**Before editing:**
```powershell
# Step 1 — find parent task and assign ID (using PowerShell on Windows)
Select-String -Path docs/master_roadmap.md -Pattern "\[.\] T-" | Select-Object -Last 10
...
# Step 3 — Run R9 3-step checks before touching any code (using grep_search tool)
```
...
Select-String -Path knowledge/error_index.md -Pattern "## ERR-" | Select-Object -Last 1
...
```powershell
# Action (using PowerShell on Windows):
Select-String -Path knowledge/index_variables.json -Pattern '"SymbolName"' -Context 0,8
# or for file lookup:
Select-String -Path knowledge/index_files.json -Pattern '"src/path/file.tsx"' -Context 0,6
```
Emit trace:
```
...
```powershell
# Action (using PowerShell on Windows):
Select-String -Path knowledge/index_variables.json -Pattern '"SymbolName"' -Context 2,20
```
Emit trace:
```
...
```powershell
# Action (using PowerShell on Windows):
Select-String -Path src/path/to/file.ts -Pattern "SymbolName","function SymbolName","const SymbolName"
```
Emit trace:
```
...
4. **Bug Fixing — search error_index first:**
   ```powershell
   Select-String -Path knowledge/error_index.md -Pattern "symptom_keyword","ERR-00" -Context 0,12 | Select-Object -First 30
   ```
...
5. **Piping — always filter before returning:**
   ```powershell
   command 2>&1 | Select-String -Pattern "error","warn","fail" | Select-Object -Last 20
   ```
