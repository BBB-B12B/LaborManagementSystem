...
**Step 3 — Char-count pattern** (append to command/process to measure output):
- PowerShell syntax:
```powershell
$result = your_command_here; $result; $thai = [regex]::Matches($result, '[\u0E00-\u0E7F]').Count; Write-Output "[chars: total=$($result.Length) thai=$thai]"
```
- Cross-platform Python syntax:
```bash
result=$(your_command_here); echo "$result"; python -c "import sys; c=sys.stdin.read(); print(f'[chars: total={len(c)} thai={sum(1 for x in c if 0x0E00 <= ord(x) <= 0x0E7F)}]')"
```
