...
## Coding Standards (Next.js & Express)
1. **Framework Strictness**: Follow standard directory conventions for new files under `backend/src/` and `frontend/src/` (e.g. `frontend/src/components/`, `frontend/src/pages/`, `backend/src/services/`, etc.).
2. **Database Integrity**: When creating Firestore schemas, models, or DB service adapters, ensure they match the Technical Requirements Document carefully.
...
4. **Aesthetics & UI**: Use Vanilla CSS styling (avoid TailwindCSS unless explicitly requested). Strive for a sleek, modern enterprise look.
...
- Source work scope: `backend/src/`, `frontend/src/`, `firebase/`, `package.json`.
