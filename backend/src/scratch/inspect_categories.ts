...
async function inspect() {
  ...
  for (const doc of catsSnap.docs) {
    const data = doc.data();
    console.log(`Category ID: "${doc.id}" | Path: "${doc.ref.path}" | Name (catName): "${data.catName}"`);
  }
  ...
  for (const doc of configsSnap.docs) {
    const data = doc.data();
    console.log(`Config ID: "${doc.id}" | Path: "${doc.ref.path}" | Name: "${data.name}" | WO Code: "${data.workOrderCode}"`);
  }
}
