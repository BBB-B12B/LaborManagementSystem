...
async function runTest() {
    try {
        ...
        const hoProject = projectsRes.data.data.find((p: any) =>
            p.projectName.toLowerCase().includes('ho') ||
            p.department === 'HO' ||
            p.code === 'HO'
        );
        ...
        console.log(`Using Project ID: ${projectId} (${hoProject ? hoProject.projectName : 'Fallback'})`);
...
200022 2023-11-12 17:00:00`;
        ...
        const textUploadRes = await axios.post(`${API_URL}/scan-data/import-text`, {
