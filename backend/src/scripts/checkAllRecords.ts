  ...
  for (const doc of snap.docs) {
    ...
    if (!data.assigneeId) {
      ...
      if (nullAssigneeNameCount <= 10) {
        console.log(
          `[Null Name] Doc: ${doc.id} | Date: ${data.workDate} | EmpId: ${data.employeeId} | EmpName: ${data.employeeName} | AssigneeId: ${data.assigneeId} | AssigneeName: ${data.assigneeName}`
        );
      }
    } else {
      ...
      if (hasAssigneeNameCount <= 5) {
        console.log(
          `[Has Name] Doc: ${doc.id} | Date: ${data.workDate} | EmpId: ${data.employeeId} | EmpName: ${data.employeeName} | AssigneeId: ${data.assigneeId} | AssigneeName: ${data.assigneeName}`
        );
      }
    }
  }
