...
async function importWageSystem() {
  ...
  const dataSummary = fs.existsSync(fileSummary)
    ? parseCSV(fs.readFileSync(fileSummary, 'utf-8'))
    : [];
  const dataPlanning = fs.existsSync(filePlanning)
    ? parseCSV(fs.readFileSync(filePlanning, 'utf-8'))
    : [];
