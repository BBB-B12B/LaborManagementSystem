            ...
            for (const task of taskSnapshot.docs) {
                ...
                for (const rev of revSnapshot.docs) {
                    ...
                    for (const rep of dailyReportsSnap.docs) {
                        await subtaskRef.collection('revisions').doc(rev.id).collection('dailyReports').doc(rep.id).set(rep.data());
                    }
                }
                ...
                for (const help of helpSnapshot.docs) {
                    ...
                    for (const rep of dailyReportsSnap.docs) {
                        await subtaskRef.collection('help').doc(help.id).collection('dailyReports').doc(rep.id).set(rep.data());
                    }
                }
