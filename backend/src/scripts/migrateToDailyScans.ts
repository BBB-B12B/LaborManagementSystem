...
async function migrate() {
    ...
    for (const [key, scans] of groups.entries()) {
        ...
        if (!contractor) {
            const dcDoc = await db.collection('dailyContractors').doc(baseScan.dailyContractorId || employeeId).get();
            if (!dcDoc.exists) {
                const dcQuery = await db.collection('dailyContractors').where('employeeId', '==', employeeId).limit(1).get();
                if (!dcQuery.empty) {
        ...
        const sortedScans = scans.filter(s => s.scanDateTime || s.workDate).sort((a, b) => {
        ...
        const allTimes = Array.from(new Set(sortedScans.map(s => {
