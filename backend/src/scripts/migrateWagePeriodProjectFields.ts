...
export async function migrateWagePeriodProjectFields() {
    ...
    try {
        ...
        for (const doc of wagePeriodsSnapshot.docs) {
            ...
            if (projectLocationId && (!data.projectCode || !data.projectName)) {
                ...
                if (project) {
                    ...
                    migratedCount++;
                    logger.info(`Migrated WagePeriod ${doc.id}: Set projectCode=${project.projectCode}, projectName=${project.projectName}`);
                } else {
                    logger.warn(`Project not found for WagePeriod ${doc.id} (projectLocationId: ${projectLocationId})`);
        ...
        logger.info(`WagePeriod Project Refactor Completed: Migrated ${migratedCount}, Skipped ${skippedCount}, Errors ${errorCount}`);
