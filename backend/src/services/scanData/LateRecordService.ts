...
class LateRecordService extends BaseCrudService<LateRecord> {
   ...
   */
  async createLateRecord(input: CreateLateRecordInput, recordedBy: string): Promise<LateRecord> {
    try {
      const { dailyContractorService } = await import('../dailyContractor/DailyContractorService');
      const compensation = await dailyContractorService.getCompensationDetails(
        input.dailyContractorId
      );
      const hourlyRate = (compensation.income as any)?.hourlyRate || 0;
      ...
      logger.info(`Late record created for DC: ${input.dailyContractorId}`, {
        recordId: record.id,
      });
  ...
  async getByPeriod(wagePeriodId: string): Promise<LateRecord[]> {
    try {
      return await this.query([{ field: 'wagePeriodId', operator: '==', value: wagePeriodId }]);
    } catch (error: any) {
