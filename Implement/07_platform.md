...
- B1 skill-marker on restore (T-325): in the compact-restore branch boot_init.sh parses compact_state.md `sk:` and appends `sk|<today>|0` to `.sessions/.active_skill`. This covers the hash-skip case — when the SKILL.md hash matches, the agent SKIPS `read_skill.py`, so no load call runs and the posttool_track.py marker writer (which now also catches a `Bash read_skill.py <bucket>/<skill>` call) never fires. Without this, skill_gate.py false-blocks the first harness-file edit even though the skill IS active. Two DISJOINT triggers (in-session load = the hook · boot-restore = here), not duplication.
