...
| `index_reconcile.py` | **Stop-hook safety net** — diffs git-changed files vs `index_files.json`, emits `[index-drift]`, auto-runs the idempotent regenerators above (rule_indexer · backlink_analyzer · code_graph · symbol_indexer) + repo_map_check `--sync`. Also `--check-labels` (T-305): read-only label-integrity pass — a per-file `topic_map` `label` not registered in `topic_registry.json` `labels_by_topic[topic]` → `[label-drift]` (advisory · always exit 0). **T-331 hook-parity:** default/Stop path AUTO-HEALS hook-list drift (`heal_hook_drift` → regenerate `hooks.json` from `settings.json` · diff-before-write · `[hook-synced]`); `--check` folds hook drift into exit-2 (`hook_drift_check`) so the close-gate blocks `phase:done` (escape `HARNESS_SKIP_PROPAGATION_BLOCK`) | Session close (Stop hook · automatic) · manual: `python3 scripts/index_reconcile.py --sync` · `--check-labels` (T-183/T-190/T-193/T-305/T-331) |
| `release.py` | **T-331 release helper** — ONE command to make a release propagation-ready: bump `.claude-plugin/plugin.json` version (patch/minor/major) + regenerate `hooks.json` from `settings.json` (via `gen_plugin_hooks`) + verify no hook drift (via `hooks_sync`) + print the commit/push/`/plugin update` checklist. Reuses those scripts as imports (single-source — no re-implement) | Human release moment → `python3 scripts/release.py <patch\|minor\|major>` · `--dry-run` (writes nothing) · `--self-test` (T-331) |
| `version_check.py` | **T-332 update detector** — SessionStart hook: compares the running harness version vs the latest released (layered · Layer A = throttled ≤1/day prompt-proof `git fetch origin main` when online+git · Layer B = newest sibling plugin-cache dir · Layer C (T-337) = machine_install engine (~/.claude, no .git) reads the `.harness_source` pointer machine_install.sh writes → git-checks the recorded source clone, so machine-installed engines are no longer blind) → prints `[harness-update]` notice + change summary + user-typed update steps. Fail-safe: never blocks boot, silent when up-to-date/offline. Reuses no other script (self-contained) | Fires automatically every SessionStart (settings.json → hooks.json) · manual: `python3 scripts/version_check.py` · `--force` (skip throttle) · `--self-test` |
| `seed_permissions.py` | **T-338 machine-wide permission baseline** — SINGLE SOURCE of the command-access allow/deny baseline (deny 5: rm/rmdir/git push/git reset/git clean · allow 24: read-only + safe tools). `merge(target, dry_run)` seeds a settings.json ADDITIVELY: never removes/weakens/reorders existing entries, dedups on exact match (idempotent), preserves every other top-level key, creates the file if absent. Plugins cannot ship a `permissions` block, so the installers are the only propagation path: machine_install.sh seeds the target's settings.json (self-host · additive+announced · honors "never touch user config"), project_init.py seeds user-global `~/.claude/settings.json` (plugin-consumer path · best-effort). Both reuse this one list — no duplication. | `python3 scripts/seed_permissions.py <target-settings.json> [--dry-run]` · `--self-test` |
...
| `cfp_fix_plan_gate.py` | **PreToolUse hard-block (T-316 · fixes CFP-048)** — on Write/Edit of `.sessions/mece_plan.md`, if the task is a CFP-fix (gather task/goal names a CFP/ERR fix OR plan `File:` touches CODING_FAILURE_PATTERNS.md) AND the plan carries no loop-closure marker → exit 2; escape `HARNESS_SKIP_CFP_PLAN_GATE=1`; fail-safe exit 0 on error | PreToolUse (matcher Edit\|Write) · registered in `.claude/settings.json` |
