...
export const CategoryConfigModal: React.FC<Props> = ({
  open,
  onClose,
  onSuccess,
  projectId,
  ...
  const handleDelete = async () => {
    ...
    const isConfirmed = await confirmDialog.confirm({
      title: 'ยืนยันการลบ',
      message:
        'คุณต้องการลบหมวดหมู่งานย่อยนี้ใช่หรือไม่? ข้อมูลงานย่อยที่เกี่ยวข้องจะถูกลบด้วย (ถ้าไม่มีการเริ่มงานไปแล้ว)',
        ...
        <Box component="form" onSubmit={handleSubmit(onSubmit)}>
          <DialogContent sx={{ px: 4, py: 2 }}>
            <Typography variant="body2" color="text.secondary" sx={{ mb: 3 }}>
              {editData
                ? 'แก้ไขชื่อหมวดหมู่งานย่อยของคุณ'
                : 'กรุณากรอกชื่อเพื่อสร้างหมวดหมู่งานย่อยใหม่'}
            </Typography>
