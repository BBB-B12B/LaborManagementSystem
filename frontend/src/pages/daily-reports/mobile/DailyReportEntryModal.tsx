...
export const DailyReportEntryModal: React.FC<DailyReportEntryModalProps> = ({
  ...
  const handleToggleWorker = (
    workerId: string,
    section: 'regular' | 'otMorning' | 'otNoon' | 'otEvening'
  ) => {
  ...
  const getOtCandidates = () => {
    return regularCandidates.filter(
      (w) => regular.workerIds.includes(w.id) || existingRegularWorkerIds.includes(w.id)
    );
  };
      ...
      {candidates.length === 0 ? (
        ...
        candidates.map((w) => (
          <ListItem
            key={w.id}
            button
            onClick={() => handleToggleWorker(w.id, inputType)}
            disablePadding
          >
      ...
      <DialogContent sx={{ p: 0, bgcolor: '#f5f5f5' }}>
        <Stack spacing={2} p={2}>
          ...
          <Card variant="outlined" sx={{ borderRadius: 2 }}>
            <CardContent
              sx={{ p: '16px !important', display: 'flex', flexDirection: 'column', gap: 2 }}
            >
              <TextField
                label="วันที่"
              ...
              >
                ...
                <Autocomplete
                  ...
                  getOptionLabel={(option) =>
                    typeof option === 'string' ? option : option.taskName
                  }
                  ...
                  renderInput={(params) => (
                    <TextField {...params} label="งาน OT" size="small" fullWidth />
                  )}
                />
                <LocalizationProvider dateAdapter={AdapterDateFns}>
                  <Box display="flex" gap={1} alignItems="center">
                    <TimePicker
                      value={otMorning.startTime}
                      onChange={(v) => setOtMorning({ ...otMorning, startTime: v })}
                      slotProps={{ textField: { size: 'small' } }}
                    />
                    <Typography>-</Typography>
                    <TimePicker
                      value={otMorning.endTime}
                      onChange={(v) => setOtMorning({ ...otMorning, endTime: v })}
                      slotProps={{ textField: { size: 'small' } }}
                    />
                  </Box>
                </LocalizationProvider>
            ...
            <Card variant="outlined" sx={{ borderRadius: 2, borderLeft: '5px solid #ed6c02' }}>
              ...
              >
                ...
                <LocalizationProvider dateAdapter={AdapterDateFns}>
                  <Box display="flex" gap={1} alignItems="center">
                    <TimePicker
                      value={otEvening.startTime}
                      onChange={(v) => setOtEvening({ ...otEvening, startTime: v })}
                      slotProps={{ textField: { size: 'small' } }}
                    />
                    <Typography>-</Typography>
                    <TimePicker
                      value={otEvening.endTime}
                      onChange={(v) => setOtEvening({ ...otEvening, endTime: v })}
                      slotProps={{ textField: { size: 'small' } }}
                    />
                  </Box>
                </LocalizationProvider>
