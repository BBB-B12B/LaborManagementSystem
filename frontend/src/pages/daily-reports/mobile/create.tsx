...
const MobileDailyReportPage = () => {
    ...
    const removeEntryMutation = useMutation({
        mutationFn: async ({ workerId, entryId }: { workerId: string; entryId: string }) => {
            if (!projectId || !date) throw new Error("Missing Project or Date");
    ...
    const handleDeleteEntry = async (workerId: string, entryId: string) => {
        if (confirm("ต้องการลบรายการนี้ใช่ไหม?")) {
            await removeEntryMutation.mutateAsync({ workerId, entryId });
        }
    };
    ...
    return (
        <Container maxWidth="sm" sx={{ p: 0, minHeight: '100vh', bgcolor: '#F4F6F8', pb: 10 }}>
            {/* Header - Sticky */}
            <Paper elevation={3} sx={{ p: 2, position: 'sticky', top: 0, zIndex: 10, borderRadius: '0 0 16px 16px' }}>
                        ...
                        {entries.map((entry: any) => (
                            ...
                            >
                                <Stack direction="row" justifyContent="space-between" alignItems="start">
                                    <Box>
                                        <Typography fontWeight="bold" variant="subtitle1">{entry.taskName}</Typography>
                                    ...
                                    <IconButton size="small" color="error" onClick={() => handleDeleteEntry(entry.dailyContractorId, entry.id)}>
