...
export const DailyReportDashboard = () => {
  ...
  const removeEntryMutation = useMutation({
    mutationFn: async (entryId: string) => {
      if (!projectId || !date) throw new Error('Missing Project or Date');
      // workerId is same as dailyContractorId. We'll find it from entries.
      ...
      >
        ...
        <Stack direction={{ xs: 'column', md: 'row' }} spacing={2}>
          ...
          <Paper sx={{ p: 0.5, borderRadius: 2, flex: 1, bgcolor: 'white' }}>
            <DatePicker label="" value={date} onChange={setDate} size="small" />
          </Paper>
        </Stack>
      </Box>
      ...
      <Box sx={{ p: 0 }}>
        {!projectId ? (
          <Alert severity="info" sx={{ borderRadius: 2 }}>
            กรุณาเลือกโครงการเพื่อเริ่มทำงาน
          </Alert>
        ) : isLoading ? (
              ...
              >
                ...
                >
                  ...
                  <Typography variant="body2" fontWeight="medium">
                    พนักงาน:{' '}
                    <span style={{ fontFamily: 'monospace' }}>{entry.dailyContractorId}</span>
                  </Typography>
                </Box>
              </Paper>
            ))}
