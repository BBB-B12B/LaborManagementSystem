...
import { MasterTask, WorkOrder, LaborRecord, TaskUpdate, Staff, Project, Contractor } from '@/types/legacy';
import { Search, Building2, HardHat, Camera, CheckCircle2, User, Users, Plus, Info, AlertCircle, LayoutDashboard, Clock, MapPin, Package, Bell, CheckSquare, Square, Loader2, Activity } from 'lucide-react';
...
const DailyReport = () => {
    ...
    return (
        <div style={{ display: 'grid', gridTemplateColumns: 'minmax(380px, 1fr) 2fr', gap: '2rem', height: 'calc(100vh - 120px)', marginTop: '20px' }}>
            ...
            <div style={{ background: '#fff', borderRadius: '24px', border: '1px solid #e2e8f0', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
                ...
                <div style={{ flex: 1, overflowY: 'auto', padding: '1rem' }}>
                    {newTasks.length === 0 && inProgressTasks.length === 0 ? <div style={{ textAlign: 'center', padding: '3rem 1rem', color: '#94a3b8' }}><div style={{ fontSize: '0.9rem', fontWeight: 700 }}>ไม่มีงานที่ต้องรายงานในขณะนี้</div></div> :
                        <>
                            {newTasks.length > 0 && <div style={{ marginBottom: '1.5rem' }}><h3 style={{ fontSize: '0.8rem', fontWeight: 800, color: '#f59e0b', marginLeft: '8px', marginBottom: '10px', textTransform: 'uppercase', display: 'flex', alignItems: 'center', gap: '6px' }}><Bell size={12} fill="currentColor" /> งานใหม่ (New Assignments)</h3>{newTasks.map(({ task, wo }) => renderTaskCard(task, wo, true))}</div>}
                            {inProgressTasks.length > 0 && <div><h3 style={{ fontSize: '0.8rem', fontWeight: 800, color: '#64748b', marginLeft: '8px', marginBottom: '10px', textTransform: 'uppercase' }}>งานที่กำลังทำ (In Progress)</h3>{inProgressTasks.map(({ task, wo }) => renderTaskCard(task, wo, false))}</div>}
                        </>
                    }
                </div>
            </div>
                ...
                {!selectedTaskInfo ? (
                    ...
                    <>
                        <div style={{ padding: '1.5rem 2rem', borderBottom: '1px solid #f1f5f9', background: '#f8fafc' }}>
                            <div style={{ background: '#fff', borderRadius: '16px', border: '1px solid #e2e8f0', overflow: 'hidden', display: 'flex' }}>
                                <div style={{ width: '160px', background: '#f1f5f9', position: 'relative' }}>
                                    {getTaskImage(selectedTaskInfo.task) ? <img loading="lazy" src={getTaskImage(selectedTaskInfo.task)!} style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <div style={{ height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#cbd5e1' }}><AlertCircle size={24} /></div>}
                                ...
                                <div style={{ flex: 1, padding: '1.25rem' }}>
                                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
                                        <div>
                                            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '4px' }}>
                                                <div style={{ fontSize: '0.7rem', fontWeight: 800, color: '#3b82f6', textTransform: 'uppercase', background: '#dbeafe', padding: '2px 8px', borderRadius: '6px' }}>{selectedTaskInfo.wo.id}</div>
                                                {(() => {
                                            ...
                                            <h2 style={{ margin: 0, fontSize: '1.25rem', fontWeight: 900, color: '#0f172a' }}>{selectedTaskInfo.task.name}</h2>
                                        </div>
                                        {(() => {
                                            const slaHoursMap = {
                                                'Immediately': 4,
                        ...
                        <div style={{ flex: 1, overflowY: 'auto', padding: '2rem' }}>
                                ...
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                                    {labor.length === 0 ? <div style={{ padding: '2rem', textAlign: 'center', border: '2px dashed #e2e8f0', borderRadius: '20px', color: '#94a3b8' }}>ยังไม่มีข้อมูลแรงงาน</div> : labor.map(l => (
                                        <div key={l.id} style={{ padding: '16px', borderRadius: '16px', background: '#fff', border: '1px solid #e2e8f0' }}>
                                                ...
                                                <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                                                    {l.membership === 'Outsource' && (
                                                        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end' }}>
                                                            ...
                                                            <input type="number" min="1" value={l.amount} onChange={(e) => setLabor(labor.map(item => item.id === l.id ? { ...item, amount: Math.max(1, parseInt(e.target.value) || 1) } : item))} style={{ width: '60px', border: '1px solid #e2e8f0', borderRadius: '8px', padding: '4px 8px', fontSize: '0.9rem', fontWeight: 700, outline: 'none', textAlign: 'center', color: '#15803d' }} />
                                                        </div>
                                                    )}
                                            ...
                                            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '20px', padding: '12px', background: '#f8fafc', borderRadius: '12px', border: '1px solid #f1f5f9' }}>
                            ...
                            <div style={{ marginTop: '2.5rem' }}>
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
                                    ...
                                    {/* Problem Toggle */}
                                    <div 
                                        onClick={() => setReportType(prev => prev === 'Problem' ? 'Normal' : 'Problem')}
                                        style={{ 
                                            display: 'flex', 
                                            alignItems: 'center', 
                                            gap: '10px', 
