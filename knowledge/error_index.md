...
## ERR-090: Geotagged-camera shutter tap is a no-op — popup closes, no handler runs, no photo attached
- **Task:** T-052b · **Session:** session_current
- **File:** frontend/src/components/camera/GeotaggedCamera.tsx · **Line:** 24 (component root — was MUI `<Dialog>`)
- **Symptom:** On a real MacBook (Firefox), the GeotaggedCamera popup opens, live video is clear, camera+location permissions granted, shutter button is visually ENABLED (bright, darkens on hover → not MUI-disabled). Tapping the shutter makes the popup vanish instantly with ZERO console output — not even the literal first line of `handleCapture` — and no photo is attached to the Daily Report.
- **Root Cause:** `GeotaggedCamera` was built on MUI `<Dialog>`, which renders through a **Portal into `document.body` as an `aria-modal`**. `PhotoSourcePicker.tsx:29-49` already documents (from prior hard experience in this codebase) that Portal / aria-modal boundaries break click / trusted-gesture handling — the tap never reaches the React `onClick` handler, so no log and no capture, and the modal tears down. The component ignored the documented non-Portal constraint of its own host component. This is why it also had to work on mobile, where Portal click-eating is worst.
- **Failed Approaches (ruled out, in order — do NOT re-test):**
  1. fullScreen Dialog "felt like a new window" → converted to small popup Dialog. Popup looked right, bug unchanged.
  2. Silent failure inside `canvas.toBlob`/capture → added `console.error` on every branch. No branch ever fired.
  3. GPS `enableHighAccuracy:true` timing out → `position` null → shutter stuck disabled → set `enableHighAccuracy:false` + retry button. User: "เหมือนเดิม" — unchanged (and confirmed shutter was enabled, so `position` was non-null).
  4. Button actually disabled / tap hitting backdrop → disproven: hover darkens the button (MUI `.Mui-disabled` sets `pointer-events:none`, so hover state ⇒ NOT disabled).
  5. Stale/desynced Next.js dev-server watcher on the network volume → killed the 2-day-old process, started fresh. Unchanged.
  6. Stale persistent webpack build cache → `rm -rf frontend/.next` + fresh restart. Unchanged.
  7. Form-submit (MUI Button default `type=submit`) → `grep` found NO `<form>`/`onSubmit` in daily-reports → ruled out.
  8. Stale bundle via service worker / PWA → `grep`/`ls` found NO service worker, next-pwa, or manifest → ruled out.
  → The common thread all 8 missed: the bug was structural (Portal), not staleness/logic — found by re-reading the host component's own documented constraint instead of chasing cache/timing ghosts.
- **Resolution:** Rewrote `GeotaggedCamera` from MUI `<Dialog>` to a plain `position:fixed; inset:0` overlay in the normal React tree (`if(!open) return null` + `<Box>` overlay + inner card) — the exact non-Portal pattern PhotoSourcePicker's bottom sheet uses. Added `type="button"` to every clickable control, a `capturing` state with on-screen "กำลังบันทึกรูป...", and on-screen red error messages on every failure branch so a fault is visible on-device without the console. `npx tsc --noEmit` EXIT=0. User confirmed the fix on device ("ปิดได้เลยครับ").

## ERR-091 · T-051 · Daily Report Log popup shows a prior revision's 100% on day-1 of a freshly-created revision
- **Symptom:** After a Leader Reject (which correctly creates a new revision with dailyProgress reset to 0), opening the "Daily Report Log" calendar popup on the new revision shows 100%/completed on day-1 — progress carried over from the previous (rejected) revision.
- **Root Cause:** Three revision-scoping failures. (1) FRONTEND (core): `TaskDailyReportModal.fetchReports` (frontend/src/page-components/workspace/components/TaskDailyReportModal.tsx) fed `getAllTaskReports` (returns EVERY revision, each tagged `_revisionId`) straight into a date-keyed `combinedReportsMap` with NO `_revisionId` filter → all revisions aggregated onto one calendar. (2) BACKEND dedup: `TaskService.getAllDailyReports` deduped with `dateMap` keyed by DATE only (`d.id`) → across two revisions sharing a date, the older won and the newer report was dropped. (3) RELATED stale field: `rejectTask` bumped subtask `currentRevision` but not `revisionId`, and both task-list builders returned `revisionId: subData.revisionId || subData.currentRevision` (stale-first).
- **Failed Approaches (ruled out — do NOT re-test):**
  1. First diagnosis targeted `frontend/src/pages/daily-reports/index.tsx` (the report-ENTRY page). Ruled out: that page already filters every calendar view by `_revisionId`. The buggy view is the Workspace task-card popup `TaskDailyReportModal.tsx`. (Roadmap ContextTask had named the right file.)
  2. dailyProgress carryover via `selectedTask.dailyProgress` → ruled out: `rejectTask` DOES reset `dailyProgress: 0` (TaskService.ts:397, :473).
  3. `getDailyReport` (per-date fetch) leaking old revision → ruled out: it IS scoped to `currentRev` (TaskService.ts:2414-2431).
- **Resolution:** (S4·core) In `fetchReports`, filter `siteReports`/`supportReports` by `_revisionId` to the current revision before mapping — `siteRevId = (task.currentRevision || task.revisionId || 'rev00').replace('help','rev')`, `supportRevId = siteRevId.replace('rev','help')`. (S3) `getAllDailyReports` dateMap now keyed by `${revDoc.id}_${d.id}` / `${hDoc.id}_${d.id}` so cross-revision same-date reports both survive. (S2) both list-builders flipped to `revisionId: subData.currentRevision || subData.revisionId || 'rev00'` (authoritative-first) — repairs already-rejected tasks with NO data migration. (S1) `rejectTask` now stamps `revisionId: nextRevId` on both subtask updates. Backend + frontend `npx tsc --noEmit` EXIT=0. Behavioral verification pending on device/staging (needs a rejected task carrying a 100% report). Out of scope: no change to approve/support flow or the dailyProgress reset (already correct).

## ERR-092 · T-053 · Logout shows White Screen — /login never loads (router-in-useEffect-deps redirect loop)
- **Symptom:** Pressing Logout → the Login page does not load; a blank White Screen appears instead. Same class of hang also hit a Foreman navigating to `/workspace` (no permission → should redirect) and any protected route opened while logged out. Console showed Next.js "Abort fetching component for route" repeatedly.
- **Root Cause:** In `frontend/src/components/layout/ProtectedRoute.tsx` the redirect `useEffect` listed `router` in its dependency array AND called `router.replace()` inside. During a client-side route transition Next.js's `router` object changes identity → the effect re-fires → it calls `router.replace('/login')` again → aborts the in-flight navigation → re-renders → repeats. The navigation never settles, so nothing paints = white screen. (A self-perpetuating redirect loop, not a permissions bug.)
- **Failed Approaches (ruled out — do NOT re-test):**
  1. "Auth state not clearing on logout" → ruled out: `handleLogout` correctly clears `authToken`/`user` and Zustand state (already fixed under ERR-088); `isAuthenticated` did flip to false. The redirect itself was firing — just looping.
  2. "Login page (`/login`) crash" → ruled out: navigating to `/login` directly (no prior auth) rendered fine; the hang only appeared on a transition driven BY ProtectedRoute's effect.
- **Resolution:** (a) Removed `router` from the effect deps → deps are now `[isAuthenticated, isLoading, user, requiredRoles]` (+ `eslint-disable-next-line react-hooks/exhaustive-deps`). (b) Added a `redirectingRef = useRef(false)` re-entrancy guard: set true before a `router.replace`, and only replace when `!redirectingRef.current && router.pathname !== target`; reset to false once authenticated so future logouts still redirect. Fixed all three symptoms (logout white-screen, Foreman→/workspace hang, logged-out protected nav) with one change. `frontend npx tsc --noEmit` EXIT=0; browser-verified: single clean redirect, 0 console errors. Out of scope: no change to auth store / logout handler / /login or /unauthorized pages.
